...
import {
  getYouTubeVideos,
  getMockLatestServices,
  VideoRender,
  Video,
} from '@/lib/services/youtube';

export async function LatestServices() {
  const isDevelopment = process.env.NODE_ENV === 'development';

  let videos: VideoRender[];

  if (isDevelopment) {
    videos = getMockLatestServices();
  } else {
    const playlistId = process.env.SERVICES_PLAYLIST_ID;
    if (!playlistId) {
      throw new Error(
        'SERVICES_PLAYLIST_ID is not defined in environment variables'
      );
    }
  ...
  return (
    <div className="flex justify-between text-left pb-4 md:pb-16 space-x-4 md:space-x-8 overflow-x-scroll scrollbar-none">
      {videos.map(
        ({
          videoId,
          thumbnailWidth,
